...
Dodecahedron::Bigint cache[51][1002] = {};

Dodecahedron::Bigint cnt(short int digits, short int sum) {
    if (cache[digits][sum] > 0) return cache[digits][sum];
    if (digits == 1) {
        return sum < 10;
    } else {
        ...
        if (!sum)
            ...
            for (short int i = min((short int) 9, sum); i >= 0; i--) {
                res += cnt(digits - 1, sum - i);
            }
        return (cache[digits][sum] = res);
    }
}
